...
class OrderNoteApp:
    ...
    def _launch_swap_updater_and_exit(self, new_exe: Path) -> bool:
        ...
        ps = (
            ...
            "$staged=$target + '.new';"
            "$backup=$target + '.bak';"
            "$log=Join-Path ([IO.Path]::GetDirectoryName($target)) 'update_launch.log';"
            "try{Add-Content -LiteralPath $log -Value ('---- ' + (Get-Date -Format s) + ' updater start ----')}catch{};"
            "for($i=0;$i -lt 90;$i++){"
            ...
            "  $launched=$false;"
            "  for($k=0;$k -lt 12 -and -not $launched;$k++){"
            ...
            "      for($w=0;$w -lt 12;$w++){"
            "        Start-Sleep -Seconds 5;"
            "        if($p.HasExited){$stable=$false;break}"
            "      };"
            "      if($p -and $stable -and -not $p.HasExited){"
            "        $launched=$true;"
            "        try{Add-Content -LiteralPath $log -Value ('launch ok attempt=' + ($k+1))}catch{}"
            "      } else {"
            "        try{Add-Content -LiteralPath $log -Value ('launch exited attempt=' + ($k+1))}catch{}"
            "      }"
            "    } catch {}"
            "  }"
            "  if(-not $launched){try{Add-Content -LiteralPath $log -Value 'launch failed after retries'}catch{}}"
            "} catch {"
            ...
            "        $stable2=$true;"
            "        for($w2=0;$w2 -lt 12;$w2++){"
            "          Start-Sleep -Seconds 5;"
            "          if($p2.HasExited){$stable2=$false;break}"
            "        };"
            "        if($p2 -and $stable2 -and -not $p2.HasExited){"
            "          try{Add-Content -LiteralPath $log -Value ('rollback launch ok attempt=' + ($k2+1))}catch{};"
            "          break"
            "        } else {"
            "          try{Add-Content -LiteralPath $log -Value ('rollback launch exited attempt=' + ($k2+1))}catch{}"
            "        }"
            "      } catch {}"
            "    }"
            "  } catch {}"
