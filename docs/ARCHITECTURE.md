...
- 主標籤篩選 PDF
  - 支援同一標籤內按「資料頁」分段排列名單
  - 支援 `(拋)` / `(自)` 標記與外框（黑白列印也可辨識）
  - 支援 UI 調整「每排人數」「名字字級」並儲存
- 交叉表 PDF
  - A4 橫印
  - 盡量不跨頁（區塊 `KeepTogether`）
  - 表末列名稱為「標籤合計」

## 10. 匯報表（暫緩）

已確認要做「匯報表」，但目前先列為待辦，規格尚未定版。

建議後續規格至少定義：

1. 輸出對象（營運/現場/主管）與閱讀順序  
2. 指標清單（例如：總人數、大/小、拋/自、資料頁分佈、異常）  
3. 口徑定義（是否去重、是否跨標籤可重複）  
4. 版型（A4 直/橫、是否單頁摘要 + 明細附表）  
5. 匯出格式（PDF only 或 PDF + CSV）
