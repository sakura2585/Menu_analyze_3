...
# 發布用完整打包：保留原 order_note.spec 作本機測試，此檔額外納入 selenium/certifi。
...
def _write_win_version_info(path: str, filevers: tuple, vs_display: str) -> None:
    ...
    content = f"""# UTF-8
# 由 order_note_full.spec 依 app.py _APP_VERSION 自動產生
VSVersionInfo(
  ffi=FixedFileInfo(
    filevers={filevers},
    prodvers={filevers},
...
_exe_options = dict(
    ...
    upx=False,
    upx_exclude=[],
    runtime_tmpdir=r"C:\Users\Public\MenuAnalyzeRuntime",
    console=False,
